      ...
      {purchaseItem?.type === "company" ? (
        ...
        <div className="std-panel">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider">Company Exams Directory</h2>
          </div>
              ...
              {(assessmentPlanFilter
                ...
                return (
                  <div
                    ...
                    className="bg-white dark:bg-[#050B14] rounded-2xl p-6 border border-slate-200 dark:border-slate-850 flex flex-col items-center justify-center text-center cursor-pointer hover:border-emerald-500 transition-all group relative min-h-[180px] shadow-sm hover:shadow-md"
                  >
                    {unlocked ? (
                      <div className="absolute top-4 right-4 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase flex items-center border border-emerald-500/20 tracking-wider">
                        <CheckCircle2 className="w-2.5 h-2.5 mr-1" />
                        Unlocked
                      </div>
                    ) : paid ? (
                      <div className="absolute top-4 right-4 bg-amber-500/10 text-amber-600 dark:text-amber-400 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase flex items-center border border-amber-500/20 tracking-wider">
                        <Lock className="w-2.5 h-2.5 mr-1" />
                        Paid
                      </div>
                    ) : (
                      <div className="absolute top-4 right-4 bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase flex items-center border border-blue-500/20 tracking-wider">
                        Free
                      </div>
                    )}
                    ...
                    {c.logoUrl ? (
                      ...
                      <div className="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 mb-4">
                        <Building2 className="w-8 h-8" />
                      </div>
                    )}
                    <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                      {c.name}
                    </h3>
                  </div>
                );
        ...
        <div className="std-panel">
          <button
            ...
            className="flex items-center text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors mb-6 uppercase tracking-wider gap-1"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to Directory
          </button>
          
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 border-b border-slate-200 dark:border-slate-850 pb-8 mb-8">
            {activeCompany.logoUrl ? (
              <img
                src={activeCompany.logoUrl}
                alt={activeCompany.name}
                className="w-24 h-24 object-contain rounded-2xl bg-white border border-slate-200 dark:border-slate-800 p-4 shrink-0 shadow-md"
              />
            ) : (
              <div className="w-24 h-24 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 shrink-0 shadow-md">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              {activeCompany.description && (
                <div className="bg-white dark:bg-[#050B14] p-5 rounded-2xl border border-slate-200 dark:border-slate-850 mt-4">
                  <h3 className="font-bold text-slate-700 dark:text-slate-350 mb-2 flex items-center text-xs uppercase tracking-wider gap-2">
                    <Info className="w-4 h-4 text-emerald-500" />
                    About placement criteria
                  </h3>
                  <div className="text-slate-600 dark:text-slate-400 leading-relaxed text-xs font-medium whitespace-pre-wrap">
                    {activeCompany.description}
                  </div>
                </div>
              )}
          ...
          <div className="mb-6 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base uppercase tracking-wider flex items-center gap-2">
            ...
            {modules
              ...
              .map((mod) => {
                ...
                return (
                  <div
                    key={mod.id}
                    className="p-6 border border-slate-200 dark:border-slate-850 rounded-2xl hover:border-slate-400 dark:hover:border-slate-700 bg-white dark:bg-[#050B14] flex flex-col justify-between relative overflow-hidden min-h-[220px] shadow-sm"
                  >
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <div className="flex flex-col">
                          <h5 className="font-bold text-slate-800 dark:text-slate-200 text-base leading-tight">
                            {mod.title}
                          </h5>
                          <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mt-1.5">
                            {mod.category}
                          </span>
                        </div>
                        {hasCompleted && (
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 my-4 line-clamp-3 leading-relaxed">
                        {mod.description}
                      </p>
                    </div>

                    <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-850/50">
                      ...
                      {access ? (
                        <button
                          ...
                          className="w-full flex items-center justify-center space-x-2 bg-emerald-600 hover:bg-emerald-700 dark:bg-[#17223b] dark:hover:bg-emerald-600 text-white py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors shadow-md"
                        >
