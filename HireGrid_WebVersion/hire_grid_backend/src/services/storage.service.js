...
if (STORAGE_PROVIDER === "s3" && config.storage.r2AccountId) {
  try {
...
function getPublicUrl(fileKey) {
  if (s3Client && config.storage.r2PublicUrl) {
    const publicUrl = config.storage.r2PublicUrl.replace(/\/$/, "");
...
async function getSignedUploadUrl(fileName, fileType) {
  ...
  if (s3Client) {
    const bucketName = config.storage.r2BucketName || "hiregrid-media";
