  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start animate-in fade-in duration-300">
      ...
      <div className="lg:col-span-1 space-y-6">
        <div className="std-panel text-center relative overflow-hidden">
          ...
          <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-emerald-500 via-teal-500 to-blue-500 p-1 mx-auto shadow-lg">
            <div className="w-full h-full rounded-full bg-[#0E1629] flex items-center justify-center text-slate-100 font-bold text-xl">
              {getInitials(currentUser?.name)}
            </div>
          </div>

          <h3 className="mt-4 font-black text-slate-100 text-lg leading-tight">
            {currentUser?.name || "Unnamed Operator"}
          </h3>
          <p className="text-xs text-slate-400 mt-1 font-mono">{currentUser?.email}</p>
          <span className="inline-block mt-3 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-bold text-[10px] uppercase tracking-wider rounded-full">
            {currentUser?.branch || "No Branch Assigned"}
          </span>

          <div className="mt-6 pt-6 border-t border-slate-850 grid grid-cols-2 gap-4 text-center">
            <div className="bg-[#050B14] p-3 rounded-xl border border-slate-900">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">XP Rank</span>
        ...
        <div className="std-panel space-y-4">
          <h4 className="font-bold text-slate-200 text-sm uppercase tracking-wider mb-2">Performance Summary</h4>
          ...
          <div className="flex items-center justify-between py-2 border-b border-slate-850">
            ...
            <span className="text-xs font-bold text-slate-100">{testsAttempted}</span>
          </div>
          ...
          <div className="flex items-center justify-between py-2 border-b border-slate-850">
            ...
            <span className="text-xs font-bold text-slate-100">{averageScore}%</span>
          </div>
          ...
          <div className="flex items-center justify-between py-2 border-b border-slate-850">
            ...
            <span className="text-xs font-bold text-slate-100">{medalInfo?.fullName || "Bronze V"}</span>
          </div>
      ...
      <div className="lg:col-span-2 std-panel space-y-6">
        <div>
          <h3 className="text-lg font-black text-slate-100 uppercase tracking-wider">Operator Profile Settings</h3>
          <p className="text-xs text-slate-400 mt-1">Keep your student credential telemetry updated.</p>
        </div>
        ...
        <form onSubmit={onSaveProfile} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-850 bg-[#050B14] text-slate-100 text-sm focus:border-emerald-500 outline-none transition-all"
                placeholder="Operator Full Name"
              />
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Academic Semester</label>
              <select
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-mono">Academic Branch</label>
            <div className="flex justify-between items-center p-4 rounded-xl border border-slate-850 bg-[#050B14]">
              ...
              <button
                ...
                className="px-3.5 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500 hover:text-white font-bold text-xs rounded-lg transition-all shrink-0"
              >
            ...
            {isChangingBranch && (
              <div className="mt-3 p-4 border border-slate-850 rounded-xl bg-[#090F1D] space-y-2 animate-in slide-in-from-top-2 duration-250">
                <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Select specialization</span>
                  ...
                  {activeBranches
                    ...
                    .map((b) => (
                      <button
                        ...
                        className="text-left px-3 py-2 text-xs font-bold border border-slate-850 bg-[#050B14] hover:border-emerald-500 hover:bg-emerald-500/5 rounded-lg transition-all text-slate-350 hover:text-slate-100"
                      >
                        {b.name}
                      </button>
                    ))}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-2 sm:col-span-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">College Name</label>
              <input
                type="text"
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-850 bg-[#050B14] text-slate-100 text-sm focus:border-emerald-500 outline-none transition-all text-center"
              />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">University Affiliation</label>
            <input
              type="text"
