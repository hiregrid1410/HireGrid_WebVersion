            ...
            {activeTab === "question-bank" && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex justify-between items-center bg-slate-900 p-6 rounded-2xl border border-slate-800">
                  <div>
                    <h2 className="text-xl font-bold text-white uppercase tracking-wider">Question Bank</h2>
                    <p className="text-xs text-slate-400 mt-1">Review, inspect, and manage active MCQs across learning modules.</p>
                  </div>
                  <button
                    onClick={() => setActiveTab("import-export")}
                    className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all"
                  >
                    Import Content
                  </button>
                </div>
                ...
                {/* Filters */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-900 p-5 rounded-2xl border border-slate-800">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs outline-none focus:border-emerald-500 text-slate-200"
                    />
                  </div>
                  <div>
                    <select
                      ...
                      className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs outline-none focus:border-emerald-500 text-slate-300"
                    >
                  ...
                  <div className="flex items-center text-xs text-slate-400 px-2 font-mono">
                    Showing {questions.length} of {questionsCount} total questions
                  </div>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                  {loadingQuestions ? (
                    <div className="p-8 text-center text-xs text-slate-500 animate-pulse">Loading Question Bank...</div>
                  ) : (
                    <DataTable
                      columns={[
            ...
            {activeTab === "import-export" && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800">
                  <h2 className="text-xl font-bold text-white uppercase tracking-wider">Import / Export Tools</h2>
                  <p className="text-xs text-slate-400 mt-1">Directly import question banks or backup learning modules in JSON format.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* JSON Paste Import Drawer */}
                  <form onSubmit={handleBulkImport} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300">Bulk JSON Import</h3>
                    <div className="space-y-2">
                      <label className="text-xs text-slate-400">Import Target Type</label>
                      <select
                        value={importType}
                        onChange={(e) => setImportType(e.target.value)}
                        className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-300 outline-none"
                      >
                        <option value="questions">Questions / MCQ Bank</option>
                      </select>
                    </div>

                    <div className="space-y-2">
                      <label className="text-xs text-slate-400">Upload Mode</label>
                      <select
                        value={importMode}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs text-slate-400">Paste JSON Content</label>
                      <textarea
                        required
                        rows="6"
                        ...
                        className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 font-mono outline-none focus:border-emerald-500"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all disabled:opacity-50"
                    >
                      {isImporting ? "Importing..." : "Run Validation & Import"}
                    </button>
                  </form>

                  {/* Instructions & Template Downloads */}
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300">Guidelines</h3>
                    <div className="space-y-4 text-xs text-slate-400 leading-relaxed">
                      <p>1. Always copy the correct target module ID to map the questions to their respective branches.</p>
                      <p>2. Keep option lists length at exactly 4 values to match student exam templates.</p>
                      <p>3. Do not modify existing companies or content indices to preserve data integrity.</p>
                    </div>

                    <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 flex items-start space-x-3">
                      <Info className="text-cyan-400 w-4 h-4 mt-0.5 shrink-0" />
                      <div>
                        <h4 className="text-xs font-bold text-white uppercase">Download templates</h4>
                        <p className="text-[10px] text-slate-500 mt-1">Use standard schemas to prevent validation errors during imports.</p>
                        <a
                          href="data:text/json;charset=utf-8,[{}]"
                          download="questions_template.json"
                          className="inline-block mt-3 text-xs text-emerald-400 hover:underline font-bold"
                        >
